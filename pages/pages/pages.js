...
Page({
  data: {
    selectArray: ['', '', ''],
    selectIndex: [0, 0, 0],
    cateArray: ['全部分类', '餐饮美食', '外卖送餐', '专业服务', '汽车服务', '便民家政', '礼品商店', '移民教育', '旅游机票', '超市商店', '医疗保健', '房产经济', '以物易物', '快递货运', '美容美发', '休闲娱乐', '酒店旅馆', '宠物服务', '家政保洁', '微商部落'],
    cateIndex: 0,
  ...
  getBusinessList() {
    wx.request({
      url: app.globalData.apiUrl + 'v6/get_business.php?cate=' + cate + '&order=' + order + '&area=' + this.data.area + '&uid=' + app.globalData.uid,
      success: res => {
        if(res.data.ret==1){
          this.setData({
            data: res.data.data
